...
#define CHUNK_HEADER_SIZE (offsetof(struct s_chunk, smaller)) // header size
#define MIN_SIZE 16 // for binary tree
#define MIN_CHUNK_SIZE (MIN_SIZE + CHUNK_HEADER_SIZE)



#define chunk2mem(p)   ((void*)((char*)(p) + CHUNK_HEADER_SIZE)) // convert a chunk ptr to a ptr on it's mem
#define mem2chunk(mem) ((chunk*)(((char*)(mem)) - CHUNK_HEADER_SIZE)) // convert a mem ptr to a ptr on it's chunk
