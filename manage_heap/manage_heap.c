...
mbool	reduce_heap(chunk *to_free)
{
	...
	if (IS_EXIST(to_free->mchunk_size)) {
		return FAILURE;
	} else if (IS_REDUCIBLE(to_free, *page_padding, my_get_page_size())) {
		*brk_addr = REDUCE_BRK(*brk_addr, to_free);
		to_release = GET_HOW_MUCH_TO_RELEASE(to_free, my_get_page_size());
		*page_padding = (*page_padding +
				clean_size(to_free->mchunk_size)) % my_get_page_size();
		remove_last(to_free);
		sbrk(-to_release);
		return SUCCESS;
	} else {
...
void	*increase_heap(size_t request)
{
	...
	if (*page_padding >= request) {
		...
		*page_padding = ((my_get_page_size()) - (adjusted_request % my_get_page_size()));
		sbrk(align_increase_heap(adjusted_request, my_get_page_size()));
	}
